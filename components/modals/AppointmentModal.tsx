...
import { ChevronLeft, User, Calendar, Tag, Clock, DollarSign, Info, PlusCircle, Repeat, X, Loader2, AlertCircle, AlertTriangle, Briefcase, CheckSquare, Mail, Trash2, Edit2, Save } from 'lucide-react';
...
const AppointmentModal: React.FC<AppointmentModalProps> = ({ appointment, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    if (selectedServices.length === 0) return setError('Selecione um serviço.');
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col relative overflow-hidden h-[90vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <main className="flex-1 p-6 space-y-6 overflow-y-auto">
          ...
          <div className="space-y-1">
            ...
            <button onClick={() => setSelectionModal('client')} className="w-full flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 text-left">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
                ...
                <input type="date" value={formData.start ? format(new Date(formData.start), 'yyyy-MM-dd') : ''} onChange={e => setFormData({...formData, start: new Date(e.target.value + 'T' + format(formData.start!, 'HH:mm'))})} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold" />
            </div>
            <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Horário</label>
                <input type="time" value={formData.start ? format(new Date(formData.start), 'HH:mm') : ''} onChange={e => { const [h,m] = e.target.value.split(':'); const d = new Date(formData.start!); d.setHours(Number(h), Number(m)); setFormData({...formData, start: d}); }} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 font-bold" />
            </div>
          </div>

          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Profissional</label>
            <button onClick={() => setSelectionModal('professional')} className="w-full flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 text-left">
          ...
          <div className="space-y-2">
             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Serviços</label>
        ...
        <footer className="p-6 border-t bg-white flex flex-col gap-4">
            <button onClick={handleSave} disabled={isSaving} className="w-full py-4 bg-orange-500 text-white rounded-2xl font-black shadow-xl flex items-center justify-center gap-2">
                {isSaving ? <Loader2 className="animate-spin" size={20}/> : <Save size={20}/>} Confirmar Agendamento
            </button>
        </footer>

        {selectionModal === 'client' && <ClientSearchModal onClose={() => setSelectionModal(null)} onSelect={c => { setFormData({...formData, client: c}); setSelectionModal(null); }} onNewClient={() => {}} />}
        {selectionModal === 'professional' && <SelectionModal title="Profissional" items={dbProfessionals} onClose={() => setSelectionModal(null)} onSelect={p => { setFormData({...formData, professional: p as any}); setSelectionModal(null); }} searchPlaceholder="Buscar..." renderItemIcon={() => <User size={20}/>} />}
        {selectionModal === 'service' && <SelectionModal title="Serviços" items={dbServices} onClose={() => setSelectionModal(null)} onSelect={s => { setSelectedServices(prev => [...prev, s as any]); setSelectionModal(null); }} searchPlaceholder="Buscar..." renderItemIcon={() => <Tag size={20}/>} />}
      </div>
    </div>
  );
};
