                ...
                <div className="p-8 space-y-6">
                    <div className="space-y-4">
                        <p className="text-slate-600 text-sm leading-relaxed text-center font-medium">
                            O horário das <b className="text-slate-800">{format(newApp.start, 'HH:mm')}</b> às <b className="text-slate-800">{format(newApp.end, 'HH:mm')}</b> choca com outro atendimento.
                        </p>
                    </div>
...
const AtendimentosView: React.FC<AtendimentosViewProps> = ({ onAddTransaction, onNavigateToCommand }) => {
    ...
    const fetchAppointments = async () => {
        ...
        try {
            ...
            if (periodType === 'Semana') { rangeStart = startOfWeek(currentDate, { weekStartsOn: 1 }); rangeEnd = endOfWeek(currentDate, { weekStartsOn: 1 }); }
            else if (periodType === 'Mês') { rangeStart = startOfMonth(currentDate); rangeEnd = endOfMonth(currentDate); }
    ...
    useEffect(() => { if (!authLoading && user && resources.length > 0) fetchAppointments(); }, [currentDate, periodType, resources, user]);
    ...
    // --- SALVAMENTO ESTRITO (RADIOGRAFIA DO DB) ---
    const handleSaveAppointment = async (app: LegacyAppointment, force: boolean = false) => {
        ...
        try {
            ...
            setToast({ message: `Erro ao salvar. Verifique IDs numéricos.`, type: 'error' }); 
                    ...
                    <div className="grid relative" style={{ gridTemplateColumns: `60px repeat(${resources.length}, minmax(${colWidth + 'px'}, 1fr))` }}>
                        <div className="sticky left-0 z-[50] bg-white border-r border-slate-200 min-w-[60px]">
                            {Array.from({ length: (END_HOUR - START_HOUR) * 60 / timeSlot }).map((_, i) => {
                                ...
                                const time = `${String(START_HOUR + Math.floor(minutes / 60)).padStart(2, '0')}:${String(minutes % 60).padStart(2, '0')}`;
                                return (
                        ...
                        {resources.map((res) => (
                            <div key={res.id} className="relative border-r border-slate-200 cursor-crosshair min-h-[960px]" onClick={(e) => { if (e.target === e.currentTarget) { /* logic to trigger new appointment */ } }}>
