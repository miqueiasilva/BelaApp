...
const CommandDetailView: React.FC<CommandDetailViewProps> = ({ commandId, onBack }) => {
    ...
    const fetchContext = async () => {
        ...
        try {
            ...
            // Resolução resiliente de nomes
            const clientInfo = Array.isArray(cmdData.clients) ? cmdData.clients[0] : cmdData.clients;
            const profInfo = Array.isArray(cmdData.team_members) ? cmdData.team_members[0] : cmdData.team_members;
            
            const resolvedClientName = clientInfo?.nome || cmdData.client_name || "Consumidor Final";
            const resolvedProfName = profInfo?.name || cmdData.professional_name || "Geral";
            ...
            setCommand({
                ...
                display_professional_name: resolvedProfName
            });
    ...
    const handleConfirmPartialPayment = async () => {
        ...
        try {
            ...
            await fetchContext(); // Atualiza histórico e saldo restante
        } catch (e: any) {
    ...
    const handleFinishCheckout = async () => {
        ...
        try {
            ...
            setToast({ message: "Comanda finalizada! ✨", type: 'success' });
            setIsLocked(true);
                    ...
                    <div className="space-y-6">
                        ...
                        {!isLocked && (
                            <div className="bg-white rounded-[48px] p-8 border border-slate-100 shadow-sm space-y-6">
                                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 flex items-center gap-2"><Tag size={14} className="text-orange-500" /> Registrar Pagamento</h4>
                                ...
                                {activeMethod ? (
                                    <div className="bg-slate-50 p-6 rounded-[32px] border-2 border-orange-500 animate-in zoom-in-95 space-y-6 shadow-inner">
                                        ...
                                        <button onClick={handleConfirmPartialPayment} disabled={isFinishing} className="w-full bg-slate-800 text-white font-black py-4 rounded-2xl shadow-lg active:scale-95 transition-all uppercase text-xs tracking-widest flex items-center justify-center gap-2">
                                            {isFinishing ? <Loader2 size={16} className="animate-spin" /> : "Confirmar Valor"}
                                        </button>
                                    </div>
                                ) : (
                        ...
                        {isLocked && (
                            <div className="bg-emerald-50 p-8 rounded-[48px] border-2 border-emerald-100 text-center space-y-4 animate-in zoom-in-95 shadow-xl">
                                ...
                                <p className="text-xs text-emerald-600 font-medium leading-relaxed">Este registro está arquivado no histórico. O faturamento foi consolidado no fluxo de caixa.</p>
