...
const ComandasView: React.FC<any> = ({ onAddTransaction, onNavigateToCommand }) => {
    ...
    const filteredTabs = tabs.filter(t => {
        ...
        const name = (clientObj?.nome || t.client_name || 'Consumidor Final').toLowerCase();
                ...
                {loading ? <div className="py-20 flex justify-center"><Loader2 className="animate-spin text-orange-500" size={40} /></div> : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-24">
                        {filteredTabs.map(tab => {
                            const clientObj = Array.isArray(tab.clients) ? tab.clients[0] : tab.clients;
                            const profObj = Array.isArray(tab.team_members) ? tab.team_members[0] : tab.team_members;
                            const displayName = clientObj?.nome || tab.client_name || 'Consumidor Final';
                            const professionalName = profObj?.name || tab.professional_name || 'Geral';
                                    ...
                                    <div className="p-5 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                                        <div className="flex items-center gap-3 min-w-0">
                                            <div className="w-10 h-10 rounded-2xl bg-orange-100 text-orange-600 flex items-center justify-center font-black text-xs flex-shrink-0 uppercase">
                                                {clientObj?.photo_url ? <img src={clientObj.photo_url} className="w-full h-full object-cover rounded-2xl" /> : displayName.charAt(0)}
                                            </div>
