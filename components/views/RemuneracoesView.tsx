...
const RemuneracoesView: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
        ...
        // 1. Busca Profissionais e suas regras de comissão
        ...
        // 2. Busca Agendamentos CONCLUÍDOS no período selecionado
  ...
  const payroll = useMemo(() => {
    return professionals.map(prof => {
      // Filtra atendimentos do profissional logado no loop
      ...
      const commissionRate = Number(prof.commission_rate) || 50; // Fallback 50%
      const netCommission = totalRevenue * (commissionRate / 100);
    ...
    <div className="p-6 h-full overflow-y-auto bg-slate-50/50 font-sans text-left">
      <header className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
            ...
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Cálculo baseado em agendamentos concluídos</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-slate-900 p-6 rounded-[32px] text-white shadow-xl shadow-slate-200">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Comissões Acumuladas</p>
            <div className="flex items-center justify-between">
                ...
                >
                    ...
                    <div className="flex items-center gap-10">
                        ...
                        <div className="text-right min-w-[140px]">
                            <p className="text-[9px] text-orange-500 font-black uppercase tracking-widest mb-0.5">Comissão Líquida</p>
                            <p className="text-2xl font-black text-orange-600">{formatBRL(item.netCommission)}</p>
                        </div>
                ...
                {expandedId === item.professional.id && (
                     <div className="bg-slate-50/50 border-t border-slate-100 p-8 animate-in slide-in-from-top-4 duration-300">
                        <div className="flex items-center gap-2 mb-6">
                            ...
                            <h4 className="text-xs font-black text-slate-600 uppercase tracking-[0.2em]">Detalhes da Produção</h4>
                        </div>
                                ...
                                <table className="w-full text-sm text-left">
                                    <thead className="text-[10px] text-slate-400 uppercase font-black bg-slate-50/50 tracking-widest border-b border-slate-100">
                                        <tr>
                                            ...
                                            <th className="px-8 py-5 text-right">Preço Bruto</th>
                        ...
                        <div className="flex justify-end items-center gap-4">
                            ...
                            <button 
                                disabled={item.appointments.length === 0}
      ...
      {payroll.length === 0 && !isLoading && (
        <div className="flex flex-col items-center justify-center py-24 text-slate-300">
            ...
            <p className="font-black uppercase tracking-[0.2em] text-xs">Sem dados financeiros para este mês</p>
        </div>
      )}
    </div>
  );
