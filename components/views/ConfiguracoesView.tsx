...
import { 
    ...
    Globe, Camera, Image as ImageIcon, Bell, Scissors, Clock, 
    Wallet, Info, Plus, Instagram, Facebook, Layout, 
    CreditCard, Hash, Map, Navigation, CheckCircle2
} from 'lucide-react';
    ...
    <div className="space-y-1.5">
        ...
        <div className="relative group">
            ...
            <input 
                ...
                className={`w-full bg-slate-50 border border-slate-200 rounded-2xl py-3.5 outline-none focus:ring-4 focus:ring-orange-50 focus:border-orange-400 transition-all font-bold text-slate-700 placeholder:text-slate-300 placeholder:font-medium ${Icon ? 'pl-12 pr-4' : 'px-4'}`}
            />
        </div>
    </div>
);
...
const ConfiguracoesView: React.FC = () => {
    ...
    const handleSaveStudio = async () => {
        ...
        try {
            ...
            delete payload.id; // Evitar conflito de PK
    ...
    return (
        <div className="h-full bg-slate-50 flex flex-col font-sans relative overflow-hidden">
            ...
            <header className="bg-white border-b border-slate-200 px-8 py-6 flex-shrink-0 z-20">
                ...
                <p className="text-slate-400 text-sm font-medium mt-1 ml-12">Gerencie as informações públicas e operacionais do seu negócio.</p>
            </header>
            ...
            <main className="flex-1 overflow-y-auto p-8 custom-scrollbar pb-32">
                <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <Card className="p-0 overflow-hidden border-slate-200 shadow-sm">
                        <div className="relative h-48 bg-slate-100">
                            ...
                            <div className="absolute -bottom-10 left-8">
                                <div className="relative group">
                                    <div className="w-28 h-28 rounded-3xl bg-white border-4 border-white shadow-2xl overflow-hidden flex items-center justify-center">
                                        {previews.logo ? (
                                    ...
                                    <button 
                                        ...
                                        className="absolute inset-0 bg-black/40 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white text-[9px] font-black uppercase"
                                    >
                                        <Plus size={20} /> Mudar
                                    </button>
                        ...
                        <div className="px-8 pt-16 pb-8 space-y-6">
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Sobre o Estúdio</label>
                                <textarea 
                                    name="presentation_text"
                                    value={studioData.presentation_text}
                                    onChange={handleInputChange}
                                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 min-h-[100px] outline-none focus:ring-4 focus:ring-orange-50 focus:border-orange-400 transition-all font-medium text-slate-600 resize-none"
                    ...
                    {/* SEÇÃO 2: CONTATO & REDES SOCIAIS */}
                    <Card title="Contato & Redes Sociais" icon={<Globe className="text-orange-500" />}>
                    ...
                    {/* SEÇÃO 3: ENDEREÇO */}
                    <Card title="Endereço & Localização" icon={<MapPin className="text-orange-500" />}>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-4">
                            ...
                            <div className="md:col-span-2">
                                <InputField label="Logradouro" name="address" value={studioData.address} onChange={handleInputChange} placeholder="Av. Paulista, Rua..." icon={Navigation} />
                            </div>
                            <div className="md:col-span-1">
                    ...
                    {/* SEÇÃO 4: HORÁRIOS */}
                    <Card title="Horários de Atendimento" icon={<Clock className="text-orange-500" />}>
                        <div className="space-y-3 mt-6">
                            {DAYS_OF_WEEK.map((day) => {
                                const config = (studioData.business_hours && studioData.business_hours[day.key]) || { active: false, start: '09:00', end: '18:00' };
                                return (
                                    <div key={day.key} className={`flex flex-col sm:flex-row items-center justify-between p-4 rounded-2xl border transition-all ${config.active ? 'bg-white border-slate-200' : 'bg-slate-50 border-transparent opacity-60'}`}>
                                        ...
                                        {config.active ? (
                                            <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                                                <div className="relative">
                                                    <input 
                                                        ...
                                                        className="bg-slate-100 border-none rounded-xl px-4 py-2 text-sm font-black text-slate-700 outline-none focus:ring-2 focus:ring-orange-200" 
                                                    />
                                                </div>
                                                <span className="text-slate-300 font-bold text-xs uppercase tracking-widest">até</span>
                                                <div className="relative">
                                                    <input 
                                                        type="time" 
                                                        value={config.end} 
                                            ...
                                            <span className="text-xs font-black text-slate-400 uppercase tracking-widest italic bg-slate-200/50 px-4 py-2 rounded-xl">Fechado</span>
                                        )}
                                    </div>
                                );
                            })}
