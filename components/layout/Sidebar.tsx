...
const Sidebar: React.FC<SidebarProps> = ({ 
    ...
    const handleNavigation = (viewId: string) => {
        // CORREÇÃO: Removida lógica de window.open externa para agenda_online
            ...
            `}>
                <div className="h-20 flex items-center px-6 gap-3 border-b border-slate-100 flex-shrink-0 justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="font-black text-slate-800 text-base leading-tight">Belaflow</h1>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">v1.2 Prod</p>
                        </div>
                    </div>
                    <button onClick={onCloseMobile} className="lg:hidden p-2 text-slate-400 hover:text-slate-600 rounded-full"><X size={20} /></button>
                </div>
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50/50">
                    <div className="flex items-center gap-3 w-full p-3 rounded-2xl bg-white border border-slate-100 shadow-sm">
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-black text-slate-700 truncate mb-1">{user?.nome || 'Usuário'}</p>
                            <p className="text-[10px] text-slate-400 font-bold uppercase truncate">{user?.papel || 'Visitante'}</p>
                        </div>
                        <button onClick={handleLogout} className="p-2 text-slate-300 hover:text-red-500 rounded-xl transition-all"><LogOut size={20} /></button>
                    </div>
                </div>
            </aside>
        </>
